...
class GenderAgeJob extends Component {
  ...
  updateList(year) {
    year && actions.teachDigits.getDigitsList(year)
  }
      ...
      return [
        {
          ...
          render: (text, record) => {
            ...
            return <span className={styles['header-style']}>
              {(!config || config.code === 'COURSEWARE_APPLY_MEDIA_OTHER') ? record.other_apply_name : config.name}
            </span>
          }
        }, {
          title: '应用情况',
...
export default smart(state => {
  return {
    data: state.teachDigits.data,
    year: state.page.year,
    coursewareMedia: state.openConfig.COURSEWARE_APPLY_MEDIA,
