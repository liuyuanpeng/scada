...
import styles from '../genderAgeJob/index.scss'
...
class GenderAgeJob extends Component {
  ...
  updateList(year, category) {
    year && category && actions.summaryGenderAge.getStudentMajorHujiList({
      currentYear: year,
      studyMode: category
    })
  }
  ...
  getData = () => {
    ...
    if (this.props.gender && this.props.age && this.props.job && this.props.data) {
      this.props.gender.forEach((item, index) => {
        data.push({
          key: item.code,
          code_one: {code: 'STUDENT_ANALYSIS_ZHUANKE', name: '专业专科层次'},
          code_two: item,
      ...
      this.props.age.forEach((item, index) => {
        data.push({
          key: item.code,
          code_one: {code: 'STUDENT_ANALYSIS_BENKE', name: '专业本科层次'},
          code_two: item,
          student: this.props.data[item.code]
        })
      })
      this.props.job.forEach((item, index) => {
        data.push({
          key: item.code,
          code_one: {code: 'STUDENT_ANALYSIS_HUJI', name: '户籍'},
          code_two: item,
          student: this.props.data[item.code]
        })
      })
  ...
  render() {
    return (
      <Page importUri={'/student/import'} onSuccess={this.onUploadOK} showYear configType="STUDY_MODE">
        <Table
          bordered
          showHeader={false}
...
export default smart(state => {
  return {
    data: state.summaryGenderAge.professionData,
    year: state.page.year,
    category: state.page.type,
    age: state.openConfig.STUDENT_ANALYSIS_BENKE,
    job: state.openConfig.STUDENT_ANALYSIS_HUJI,
    gender: state.openConfig.STUDENT_ANALYSIS_ZHUANKE
  }
})(GenderAgeJob)
